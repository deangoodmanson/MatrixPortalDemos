...
def open_console_port(port: str) -> serial.Serial:
    """Open the Matrix Portal CDC console port for reading SNAP commands.

    The console port is the lower-numbered of the two CDC ports the Matrix
    Portal exposes when usb_cdc.enable(console=True, data=True) is active
    in boot.py.  Baud rate is irrelevant for USB CDC (115200 is conventional).
